...
impl Node {
    pub fn new(index: usize) -> RefCell<Node> {
        RefCell::new(Node {
            index: index,
            hash:  None,
            pos:   None,
    ...
    pub fn set_hash(&mut self, hash_for_none: u64) -> u64 {
        ...
        match &self.neg {
            ...
            Some(tree) => {
                let neg_hash = tree.borrow().hash.clone();
                match neg_hash {
                    None => {
        ...
        match &self.pos {
            ...
            Some(tree) => {
                let pos_hash = tree.borrow().hash.clone();
                match pos_hash {
                    None => {
                        let _hash = tree.borrow_mut()
                            .set_hash(hash_for_none);
    ...
    fn rehash_to_none(&mut self) {
        self.hash = None;

        if let Some(neg) = &self.neg {
            if neg.borrow().hash.is_some() {
                neg.borrow_mut().rehash_to_none();
            }
        }

        if let Some(pos) = &self.pos {
            if pos.borrow().hash.is_some() {
                pos.borrow_mut().rehash_to_none();
            }
        }
    }

    // /// Update the `self.hash` and return it.
    pub fn rehash(&mut self, hash_for_none: u64) -> u64 {
        self.rehash_to_none();
        self.set_hash(hash_for_none)
    }
...
// pub fn topological_sort(roots:  &[Rc<RefCell<Node>>],
//                         stumps: &[Rc<RefCell<Node>>],
//                         leaf:   &Rc<RefCell<Node>>)
//     -> Vec<Rc<RefCell<Node>>>
// {
...
pub fn topological_sort(root: &Rc<RefCell<Node>>,
                        leaf: &Rc<RefCell<Node>>)
    -> Vec<Rc<RefCell<Node>>>
{
    let mut que = VecDeque::new();
